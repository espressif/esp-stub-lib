...
extern int ets_sha_init(void *ctx, int type);
extern void ets_sha_update(void *ctx, const void *data, uint32_t data_len, bool update_ctx);
extern int ets_sha_finish(void *ctx, uint8_t *digest);
...
void __attribute__((weak)) stub_target_sha256_data(const void *data, uint32_t data_len)
{
