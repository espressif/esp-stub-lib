...
/**
 * Select chunk size for unencrypted write (max 32 bytes)
 */
...
/**
 * Select block size for encrypted write (16, 32, or 64 bytes based on alignment)
 */
static inline uint8_t get_write_block_size_encrypted(uint32_t flash_addr, uint32_t remaining, uint32_t page_space)
{
    if (IS_ALIGNED(flash_addr, 64) && remaining >= 64 && page_space >= 64) {
        return 64;
    } else if (IS_ALIGNED(flash_addr, 32) && remaining >= 32 && page_space >= 32) {
        return 32;
    }
    return 16;
}
...
static int stub_target_flash_4byte_write_encrypted(int spi_num, uint32_t flash_addr, const uint8_t *data, uint32_t size,
                                                   uint64_t timeout_us)
{
    if (!IS_ALIGNED(flash_addr, 16) || !IS_ALIGNED(size, 16)) {
