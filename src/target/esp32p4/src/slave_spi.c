...
static lldesc_spi_t s_dmadesc_tx;
static lldesc_spi_t s_dmadesc_rx[SPI_SLV_RX_DESC_COUNT];
...
/* P4 AXI-DMA is coherent with L2 but not L1 D-cache, so DMA buffers/descriptors
 * only need L1 D-cache maintenance (the shared stub_target_cache_* helpers also
 * touch L2). */
...
static inline void dcache_writeback(const void *addr, uint32_t size)
{
    Cache_WriteBack_Addr(CACHE_MAP_L1_DCACHE, (uint32_t)(uintptr_t)addr, size);
}

static inline void dcache_invalidate(const void *addr, uint32_t size)
{
    Cache_Invalidate_Addr(CACHE_MAP_L1_DCACHE, (uint32_t)(uintptr_t)addr, size);
}

/* Writeback + invalidate: no dirty line can be evicted over received bytes. */
static inline void dcache_flush(const void *addr, uint32_t size)
{
    dcache_writeback(addr, size);
...
static void spi_slv_rxdma_arm(uint8_t *buf, uint32_t len)
{
    ...
    while (remaining) {
        ...
        s_dmadesc_rx[desc_idx].size = chunk & LLDESC_SPI_SIZE_MASK;
        s_dmadesc_rx[desc_idx].length = chunk & LLDESC_SPI_SIZE_MASK;
        s_dmadesc_rx[desc_idx].offset = 0;
        s_dmadesc_rx[desc_idx].buf = buf_pos;
        s_dmadesc_rx[desc_idx].sosf = 0;
        s_dmadesc_rx[desc_idx].owner = 1;
        if (remaining <= chunk) {
            s_dmadesc_rx[desc_idx].eof = 1;
            s_dmadesc_rx[desc_idx].next = NULL;
        } else {
            s_dmadesc_rx[desc_idx].eof = 0;
            s_dmadesc_rx[desc_idx].next = &s_dmadesc_rx[desc_idx + 1];
        }
        remaining -= chunk;
        buf_pos += chunk;
        desc_idx++;
    }
    dcache_writeback(s_dmadesc_rx, desc_idx * sizeof(s_dmadesc_rx[0]));
    /* Flush the RX buffer before DMA so a later eviction of a dirty line can't
     * clobber received bytes (buffer is cache-line aligned). */
    ...
    WRITE_PERI_REG(AXI_DMA_IN_LINK2_CH0_REG, (uint32_t)(uintptr_t)s_dmadesc_rx);
...
bool stub_target_slave_spi_take_rx_frame(size_t *out_len)
{
    ...
    /* RX_DONE only means the SPI transaction finished; the DMA engine may still
     * be writing to RAM. Like ESP-ROM, wait for IN_SUC_EOF before reading/
     * invalidating; on timeout leave the frame pending and retry. */
    uint64_t dma_timeout_us = SPI_SLV_DMA_DONE_TIMEOUT_US;
    if (stub_target_wait_reg_bit_set(AXI_DMA_IN_INT_RAW_CH0_REG, AXI_DMA_IN_SUC_EOF_CH0_INT_RAW, &dma_timeout_us) !=
        STUB_LIB_OK) {
        return false;
    }
    WRITE_PERI_REG(AXI_DMA_IN_INT_CLR_CH0_REG, AXI_DMA_IN_SUC_EOF_CH0_INT_CLR);

    WRITE_PERI_REG(SPI_DMA_INT_CLR_REG(SPI_SLV_HOST), SPI_SLV_RX_DONE_CLR);

    /* Stub runs from cached L2MEM; drop stale lines so we read the DMA'd bytes. */
    if (s_rx_buf != NULL) {
        dcache_invalidate(s_rx_buf, s_rx_len);
    }
...
int stub_target_slave_spi_tx_frame(const void *data, size_t len)
{
    ...
    s_dmadesc_tx.size = (uint32_t)len & LLDESC_SPI_SIZE_MASK;
    s_dmadesc_tx.length = (uint32_t)len & LLDESC_SPI_SIZE_MASK;
    s_dmadesc_tx.buf = (uint8_t *)data;
    s_dmadesc_tx.sosf = 0;
    s_dmadesc_tx.eof = 1;
    s_dmadesc_tx.owner = 1;
    s_dmadesc_tx.next = NULL;
    dcache_writeback(data, (uint32_t)len);
    dcache_writeback(&s_dmadesc_tx, sizeof(s_dmadesc_tx));
    ...
    WRITE_PERI_REG(AXI_DMA_OUT_LINK2_CH0_REG, (uint32_t)(uintptr_t)&s_dmadesc_tx);
    ...
    /* SPI_SLV_TX_DONE only reports the SPI-slave RDDMA transaction; wait for the
     * AXI-DMA engine's OUT_DONE too, as ESP-ROM does, before treating the frame
     * as fully copied out of RAM. */
    timeout_us = SPI_SLV_DMA_DONE_TIMEOUT_US;
    ret = stub_target_wait_reg_bit_set(AXI_DMA_OUT_INT_RAW_CH0_REG, AXI_DMA_OUT_DONE_CH0_INT_RAW, &timeout_us);
    if (ret != STUB_LIB_OK) {
        s_rx_armed = false;
        return STUB_LIB_FAIL;
    }
    WRITE_PERI_REG(AXI_DMA_OUT_INT_CLR_CH0_REG, AXI_DMA_OUT_DONE_CH0_INT_CLR);
