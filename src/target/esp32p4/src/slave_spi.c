...
static void spi_slv_rxdma_arm(uint8_t *buf, uint32_t len)
{
    ...
    while (remaining) {
        ...
        dmadesc_rx[desc_idx].size = chunk & LLDESC_SPI_SIZE_MASK;
        dmadesc_rx[desc_idx].length = chunk & LLDESC_SPI_SIZE_MASK;
    ...
    dcache_writeback(dmadesc_rx, desc_idx * sizeof(dmadesc_rx[0]));
...
int stub_target_slave_spi_tx_frame(const void *data, size_t len)
{
    ...
    dcache_writeback(&dmadesc_tx, sizeof(dmadesc_tx));
