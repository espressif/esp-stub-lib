...
{
    return sizeof(esp32_cache_state_t);
}
...
{
    bool pro_cache_was_enabled = stub_target_cache_is_enabled();

    if (state) {
        esp32_cache_state_t *s = state;
        /* Save PRO CTRL1 register so we can restore bus mask later */
        s->pro_ctrl1 = REG_READ(DPORT_PRO_CACHE_CTRL1_REG);
        s->pro_cache_was_enabled = pro_cache_was_enabled;
    }

    if (!pro_cache_was_enabled) {
