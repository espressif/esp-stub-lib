...
void stub_target_slave_spi_init(void)
{
    ...
    WRITE_PERI_REG(SPI_DMA_INT_ENA_REG, 0);
...
static void spi_slv_rxdma_arm(uint8_t *buf, uint32_t len)
{
    ...
    while (remaining) {
        ...
        s_dmadesc_rx[desc_idx].size = dma_len & LLDESC_SPI_SIZE_MASK;
        s_dmadesc_rx[desc_idx].length = dma_len & LLDESC_SPI_SIZE_MASK;
    ...
    WRITE_PERI_REG(SPI_DMA_IN_LINK_REG, ((uint32_t)(uintptr_t)s_dmadesc_rx & SPI_INLINK_ADDR) | SPI_DMA_RX_ENA);
...
int stub_target_slave_spi_tx_frame(const void *data, size_t len)
{
    ...
    WRITE_PERI_REG(SPI_DMA_OUT_LINK_REG, ((uint32_t)(uintptr_t)&s_dmadesc_tx & SPI_OUTLINK_ADDR) | SPI_DMA_TX_ENA);
