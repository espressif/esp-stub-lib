...
{
    Cache_Resume_ICache(s_saved_autoload & BIT(0) ? EXTMEM_PRO_ICACHE_AUTOLOAD_ENA : 0);
    Cache_Resume_DCache(s_saved_autoload & BIT(1) ? EXTMEM_PRO_DCACHE_AUTOLOAD_ENA : 0);
}

int stub_target_cache_is_enabled(void)
{
    return REG_GET_BIT(EXTMEM_PRO_ICACHE_CTRL_REG, EXTMEM_PRO_ICACHE_ENABLE) &&
           !REG_GET_BIT(EXTMEM_PRO_ICACHE_CTRL1_REG, EXTMEM_PRO_ICACHE_MASK_DROM0);
}
