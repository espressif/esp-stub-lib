 ...
 * esp_rom_check_recover_key() only recovers the key when SPI_BOOT_CRYPT_CNT is
 * set, and the 4-byte write path drives the peripheral directly instead of going
 * through esp_rom_spiflash_write_encrypted(). Encrypted writes forced in that
 * state (esptool --ignore-flash-enc-efuse) would otherwise run with no key
 * loaded and silently store unusable ciphertext.
 *
 * esp_rom_recover_key() recovers regardless of SPI_BOOT_CRYPT_CNT and is a no-op
 * repeat when the key is already routed.
 */
