...
#define ESP32S31_STUB_MMU_DROM_ENTRY_START 2U

uint32_t stub_target_mmu_get_page_size(void)
{
    uint32_t page_size = REG_GET_FIELD(SPI_MEM_C_MMU_POWER_CTRL_REG, SPI_MMU_PAGE_SIZE);

    /* Page size for flash mmu is 256KB, 128KB, 64KB and 32KB */
    if (page_size == 0) {
        return STUB_MMU_PAGE_SIZE_256KB;
    } else if (page_size == 1) {
        return STUB_MMU_PAGE_SIZE_128KB;
    } else if (page_size == 2) {
...
{
    return ESP32S31_STUB_MMU_DROM_ENTRY_START;
}
...
{
    REG_WRITE(SPI_MEM_C_MMU_ITEM_INDEX_REG, entry_id);
    uint32_t mmu_raw_value = REG_READ(SPI_MEM_C_MMU_ITEM_CONTENT_REG);
    if (stub_target_security_flash_is_encrypted())
        mmu_raw_value &= ~SOC_MMU_FLASH_SENSITIVE;
...
{
    if (raw_value & SOC_MMU_FLASH_VALID)
        stub_target_mmu_write_entry(entry_id, raw_value & SOC_MMU_FLASH_VALID_VAL_MASK);
    else
...
bool stub_target_mmu_has_valid_entry(void)
{
    for (uint32_t i = 0; i < SOC_MMU_ENTRY_NUM; i++) {
        ...
        if ((raw & SOC_MMU_FLASH_VALID) && !(raw & SOC_MMU_ACCESS_PSRAM))
            return true;
    }
    return false;
}
