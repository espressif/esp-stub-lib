...
{
    return sizeof(esp32h21_cache_state_t);
}
...
void stub_target_cache_init(void *state)
{
    ...
    if (state) {
        esp32h21_cache_state_t *s = state;
        s->mmu_page_size = REG_GET_FIELD(SPI_MEM_MMU_POWER_CTRL_REG(0), SPI_MEM_MMU_PAGE_SIZE);
        s->ctrl = REG_READ(CACHE_L1_CACHE_CTRL_REG);
        s->cache_was_enabled = cache_was_enabled;
        STUB_LOGD("mmu_page_size: %d cache_ctrl: 0x%x\n", s->mmu_page_size, s->ctrl);
    }
