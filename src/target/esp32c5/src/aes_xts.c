...
void stub_target_aes_xts_encrypt_trigger(
    uint32_t flash_addr,
    void *data,
    uint32_t block_size)
{
...
#define AES_XTS_STATE_VISIBLE     0x3
    ...
    while (REG_READ(SPI_MEM_XTS_STATE_REG(INTERNAL_SPI_NUM)) != AES_XTS_STATE_DONE && timeout_us > 0) {
        timeout_us--;
    }
    if (timeout_us == 0) {
        return -1; /* Timeout waiting for encryption */
    }
    ...
    while (REG_READ(SPI_MEM_XTS_STATE_REG(INTERNAL_SPI_NUM)) != AES_XTS_STATE_VISIBLE && timeout_us > 0) {
        timeout_us--;
    }
    if (timeout_us == 0) {
        return -1; /* Timeout waiting for release */
    }
    return 0;
#undef AES_XTS_STATE_IDLE
#undef AES_XTS_STATE_BUSY
#undef AES_XTS_STATE_DONE
#undef AES_XTS_STATE_VISIBLE
}
