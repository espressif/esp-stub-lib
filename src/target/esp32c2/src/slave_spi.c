...
bool stub_target_slave_spi_take_rx_frame(size_t *out_len)
{
    ...
    if (stub_target_wait_reg_bit_set(GDMA_INT_RAW_CH0_REG, GDMA_IN_SUC_EOF_CH0_INT_RAW, &dma_timeout_us) !=
        STUB_LIB_OK) {
        return false;
    }
    WRITE_PERI_REG(GDMA_INT_CLR_CH0_REG, GDMA_IN_SUC_EOF_CH0_INT_CLR);
...
int stub_target_slave_spi_tx_frame(const void *data, size_t len)
{
    ...
    ret = stub_target_wait_reg_bit_set(GDMA_INT_RAW_CH0_REG, GDMA_OUT_DONE_CH0_INT_RAW, &timeout_us);
    if (ret != STUB_LIB_OK) {
        s_rx_armed = false;
        return STUB_LIB_FAIL;
    }
    WRITE_PERI_REG(GDMA_INT_CLR_CH0_REG, GDMA_OUT_DONE_CH0_INT_CLR);
