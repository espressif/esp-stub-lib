...
void stub_target_aes_xts_encrypt_trigger(uint32_t flash_addr, const void *data, uint32_t block_size)
{
    ...
    stub_lib_memcpy((void *)(AES_XTS_PLAIN_BASE + plaintext_offs), data, block_size);
...
int stub_target_aes_xts_wait_data_ready(uint64_t timeout_us)
{
    ...
    while (REG_READ(AES_XTS_STATE_REG) != AES_XTS_STATE_DONE && timeout_us > 0) {
        stub_lib_delay_us(1);
        --timeout_us;
    }
    if (timeout_us == 0) {
    ...
    while (REG_READ(AES_XTS_STATE_REG) != AES_XTS_STATE_VISIBLE && timeout_us > 0) {
        stub_lib_delay_us(1);
        --timeout_us;
    }
    if (timeout_us == 0) {
