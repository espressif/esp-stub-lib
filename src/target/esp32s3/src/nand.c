...
static int spi_nand_init(uint32_t hspi_arg)
{
    ...
    if (use_iomux) {
        ...
        for (int i = 0; i < 6; i++) {
            // MCU_SEL = 4 (FSPI) — no ROM function for this, must set manually
            ...
            gpio_pad_input_enable(all_pins[i]);
            gpio_pad_set_drv(all_pins[i], 2);
        }
...
int stub_target_nand_read_bbm(uint32_t page_number, uint8_t *spare_data)
{
    ...
    ret = spi_nand_transaction(NAND_CMD_READ_FROM_CACHE, col_addr << 8, 24, NULL, 0, spare_data, 32);
    if (ret != 0) {
        return -30;
    }
