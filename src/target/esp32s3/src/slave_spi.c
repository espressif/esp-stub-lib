...
static void spi_slv_rxdma_arm(uint8_t *buf, uint32_t len)
{
    ...
    WRITE_PERI_REG(GDMA_IN_LINK_CH0_REG, (uint32_t)(uintptr_t)s_dmadesc_rx & GDMA_INLINK_ADDR_CH0);
...
bool stub_target_slave_spi_take_rx_frame(size_t *out_len)
{
     ...
     * be writing to RAM. Like ESP-ROM, wait for IN_SUC_EOF before reading; on
     * timeout leave the frame pending and retry rather than read a torn frame. */
    uint64_t dma_timeout_us = SPI_SLV_DMA_DONE_TIMEOUT_US;
    if (stub_target_wait_reg_bit_set(GDMA_IN_INT_RAW_CH0_REG, GDMA_IN_SUC_EOF_CH0_INT_RAW, &dma_timeout_us) !=
        STUB_LIB_OK) {
        return false;
    }
    WRITE_PERI_REG(GDMA_IN_INT_CLR_CH0_REG, GDMA_IN_SUC_EOF_CH0_INT_CLR);
...
int stub_target_slave_spi_tx_frame(const void *data, size_t len)
{
    ...
    WRITE_PERI_REG(GDMA_OUT_LINK_CH0_REG, (uint32_t)(uintptr_t)&s_dmadesc_tx & GDMA_INLINK_ADDR_CH0);
     ...
     * GDMA engine's OUT_DONE too, as ESP-ROM does, before treating the frame as
     * fully copied out of RAM. */
    timeout_us = SPI_SLV_DMA_DONE_TIMEOUT_US;
    ret = stub_target_wait_reg_bit_set(GDMA_OUT_INT_RAW_CH0_REG, GDMA_OUT_DONE_CH0_INT_RAW, &timeout_us);
    if (ret != STUB_LIB_OK) {
        s_rx_armed = false;
        return STUB_LIB_FAIL;
    }
    WRITE_PERI_REG(GDMA_OUT_INT_CLR_CH0_REG, GDMA_OUT_DONE_CH0_INT_CLR);
