...
static inline void diag_set_stage(uint8_t stage) { s_diag.stage = stage; }
static inline void diag_capture_rintsts(void)    { s_diag.rintsts = REG_READ(SDMMC_RINTSTS_REG); }
...
static int update_clk_reg(uint8_t slot)
{
    ...
    uint32_t cmd = SDMMC_CMD_UPDATE_CLK_REG | SDMMC_CMD_WAIT_PRVDATA
                 | SDMMC_CMD_USE_HOLD_REG | SDMMC_CMD_START_CMD
...
{
    if (pin >= MAX_GPIO_NUM) return SDMMC_ERR_PIN_INVALID;
...
{
    REG_WRITE(SDMMC_CTRL_REG,
              SDMMC_CTRL_CONTROLLER_RESET | SDMMC_CTRL_DMA_RESET | SDMMC_CTRL_FIFO_RESET);
    int timeout = 5000;
    ...
    REG_WRITE(SDMMC_CTRL_REG,
              REG_READ(SDMMC_CTRL_REG) | SDMMC_CTRL_DMA_ENABLE | SDMMC_CTRL_USE_INTERNAL_DMA);
...
static uint32_t cmd_flags_for_response(resp_type_t r)
{
    ...
    if (r != RESP_NONE) {
        ...
        if (r != RESP_R3) flags |= SDMMC_CMD_RESP_CRC;  // R3 (OCR) has no CRC
        if (r == RESP_R2) flags |= SDMMC_CMD_RESP_LONG;
    }
    return flags;
}
...
static int wait_cmd_done(uint32_t *rintsts_out)
{
    ...
    while (1) {
        ...
        if (st & SDMMC_INT_CMD_DONE) {
            s_diag.rintsts = st;
            if (rintsts_out) *rintsts_out = st;
            REG_WRITE(SDMMC_RINTSTS_REG, st & (SDMMC_INT_CMD_DONE | SDMMC_INT_CMD_ERR));
            return 0;
        }
        if (--timeout <= 0) { s_diag.rintsts = st; return SDMMC_ERR_TIMEOUT; }
        stub_lib_delay_us(10);
    }
}
    ...
    while (1) {
        ...
        if (st & SDMMC_INT_DTO) {
            ...
            REG_WRITE(SDMMC_RINTSTS_REG,
                      st & (SDMMC_INT_DTO | SDMMC_INT_RXDR | SDMMC_INT_TXDR | SDMMC_INT_DATA_ERR));
            return 0;
        }
        if (--timeout <= 0) { s_diag.rintsts = st; return SDMMC_ERR_TIMEOUT; }
        stub_lib_delay_us(10);
...
static int send_cmd(uint8_t idx, uint32_t arg, resp_type_t r, bool send_init, uint32_t *resp_out)
{
    ...
    if (send_init) cmd |= SDMMC_CMD_SEND_INIT;
    ...
    if (r == RESP_R1B) return wait_card_idle();
    return 0;
}
...
static int xfer_single_block(uint8_t cmd_idx, uint32_t arg, void *buf, bool is_write)
{
    ...
    if ((((uintptr_t)buf) & 0x3U) != 0) return SDMMC_ERR_BAD_ARG;
    ...
    while (REG_READ(SDMMC_CTRL_REG) & SDMMC_CTRL_FIFO_RESET) {
        if (--timeout <= 0) return SDMMC_ERR_TIMEOUT;
        stub_lib_delay_us(1);
    }
    ...
    s_desc.ctrl         = DESC_CTRL_OWNED_BY_IDMAC | DESC_CTRL_FIRST | DESC_CTRL_LAST
                        | DESC_CTRL_END_OF_RING | DESC_CTRL_DISABLE_INT_ON_COMP;
    ...
    REG_WRITE(SDMMC_RINTSTS_REG,
              SDMMC_INT_DTO | SDMMC_INT_CMD_DONE | SDMMC_INT_CMD_ERR | SDMMC_INT_DATA_ERR
    ...
    cmd |= SDMMC_CMD_USE_HOLD_REG | SDMMC_CMD_WAIT_PRVDATA | SDMMC_CMD_START_CMD;
    cmd |= ((uint32_t)s_host.slot & 0x1FU) << 16;
    if (is_write) cmd |= SDMMC_CMD_WRITE;
    ...
    err = wait_cmd_done(&rintsts);
    if (err) return err;
    if (rintsts & SDMMC_INT_CMD_ERR) return SDMMC_ERR_CMD_FAIL;

    err = wait_data_done(&rintsts);
    if (err) return err;
    if (rintsts & SDMMC_INT_DATA_ERR) return SDMMC_ERR_DATA_FAIL;

    if (is_write) return wait_card_idle();
    return 0;
}
...
static int card_init(uint8_t target_width)
{
    ...
    if (err == 0 && (resp[0] & 0xFFU) == CMD8_PATTERN) sd_v2 = true;
    ...
    if (!got_sd) {
        ...
        if (!(resp[0] & BIT(31))) return SDMMC_ERR_NO_CARD;
    }
    ...
    // CSD v2.0 (SDHC/SDXC, eMMC ≥ 2GB): capacity = (C_SIZE + 1) * 512 KiB.
    // CSD v1.0 (SDSC) and eMMC EXT_CSD path not derived here; the host can
    // still address sectors via byte offsets for SDSC (is_high_capacity=0).
    uint32_t csd_struct = (s_card.csd[3] >> 22) & 0x3U;
    if (csd_struct == 1U) {
        uint64_t c_size = ((uint64_t)s_card.csd[1] >> 16) | (((uint64_t)s_card.csd[2] & 0x3FU) << 16);
        s_card.capacity_bytes = (c_size + 1U) * 512U * 1024U;
    } else {
        s_card.capacity_bytes = 0;
    }
    ...
    if (!s_card.is_mmc && target_width == 4U) {
        ...
        if (err) return err;
        REG_WRITE(SDMMC_CTYPE_REG, (REG_READ(SDMMC_CTYPE_REG) & ~(1U << (s_host.slot + 16)))
                                    | (1U << s_host.slot));
        s_card.width = 4;
    } else if (target_width == 8U) {
        return SDMMC_ERR_UNSUPPORTED;
    } else {
        REG_WRITE(SDMMC_CTYPE_REG, REG_READ(SDMMC_CTYPE_REG)
                                    & ~((1U << s_host.slot) | (1U << (s_host.slot + 16))));
        s_card.width = 1;
    }
...
int stub_target_sdmmc_attach(const stub_target_sdmmc_attach_config_t *cfg)
{
    ...
    if (cfg == NULL || cfg->slot == 0xFFU) {
        ...
        local = *cfg;
        if (local.width != 1 && local.width != 4 && local.width != 8) return SDMMC_ERR_BAD_ARG;
        if (local.freq_khz == 0) local.freq_khz = DEFAULT_FREQ_KHZ;
    }
    if (local.slot > 1) return SDMMC_ERR_BAD_ARG;
...
{
    if (!s_host.initialized) return SDMMC_ERR_NOT_INITIALIZED;
...
{
    if (!s_host.initialized) return SDMMC_ERR_NOT_INITIALIZED;
    if (end_lba < start_lba) return SDMMC_ERR_BAD_ARG;
...
{
    if (cmd_idx_out) *cmd_idx_out = s_diag.stage;
    if (rintsts_out) *rintsts_out = s_diag.rintsts;
}
