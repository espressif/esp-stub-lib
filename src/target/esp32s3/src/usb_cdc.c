...
// ESP32S3 needs a wrapper for the ISR handler (workaround for compressed flashing)
...
#define INTERRUPT_CORE0_USB_INTR_MAP_REG (0x600c2000 + 0x098)
...
void stub_target_usb_cdc_init(void (*rx_callback)(uint8_t))
{
    ...
    // ESP32S3: Route interrupt using direct register write
    WRITE_PERI_REG(INTERRUPT_CORE0_USB_INTR_MAP_REG, ETS_USB_INUM);

    // Attach and enable ISR (with wrapper for compressed flashing workaround)
    usb_cdc_common_attach_isr(ETS_USB_INUM, usb_dw_isr_handler_wrapper);
