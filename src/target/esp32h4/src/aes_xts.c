...
/*
 * Make sure the XTS key is routed into the peripheral.
 *
 * With flash encryption enabled the ROM already does this at boot, so the key is
 * in place by the time the stub runs. With flash encryption disabled it does not:
 * the key manager is left powered down, and the 4-byte write path drives the
 * peripheral directly instead of going through esp_rom_spiflash_write_encrypted(),
 * which recovers the key itself. Encrypted writes forced in that state
 * (esptool --ignore-flash-enc-efuse) would otherwise run with no key loaded and
 * silently store unusable ciphertext.
 *
 * esp_rom_recover_key() powers up the key manager and routes the key regardless of
 * SPI_BOOT_CRYPT_CNT, and is a no-op repeat when the key is already routed.
 */
extern int esp_rom_recover_key(int do_log);

void stub_target_aes_xts_init(void)
{
    esp_rom_recover_key(0);
