...
#define ESP32H4_STUB_MMU_DROM_ENTRY_START 2U
...
{
    return ESP32H4_STUB_MMU_DROM_ENTRY_START;
}
...
{
    uint32_t mmu_raw_value = flash_page_num | SOC_MMU_ACCESS_FLASH;
...
{
    REG_WRITE(SPI_MEM_MMU_ITEM_INDEX_REG(0), entry_id);
    REG_WRITE(SPI_MEM_MMU_ITEM_CONTENT_REG(0), raw_value);
}
...
bool stub_target_mmu_has_valid_entry(void)
{
    for (uint32_t i = 0; i < SOC_MMU_ENTRY_NUM; i++) {
        ...
        if ((mmu_raw_value & SOC_MMU_VALID) && !(mmu_raw_value & SOC_MMU_ACCESS_PSRAM))
            return true;
    }
    return false;
}
