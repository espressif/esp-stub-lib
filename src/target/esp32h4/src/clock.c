...
/*
 * CPU clock sources: 0 = XTAL (32 MHz), 1 = RC_FAST,
 * 2 = XTAL_X2 (64 MHz), 3 = PLL (96 MHz).
 */
#define SOC_CPU_CLK_SRC_PLL 3U

/* 96 MHz is the highest frequency ESP32-H4 supports (PLL, CPU divider of 1). */
...
/*
 * The core rail is supplied by a switching DC-DC converter. Out of reset it runs
 * in its default mode, which only has to carry the 32 MHz the chip boots at, so
 * it has to be brought up before the CPU clock is raised.
 */

/* Output voltage setting, shared by both conduction modes. */
#define DCDC_DREG_VALUE     24U
...
/* Selects the converter on the analog I2C bus and returns the master to use. */
static uint32_t regi2c_select_dcdc(void)
{
    uint32_t sel = REG_GET_BIT(I2C_ANA_MST_ANA_CONF2_REG, REGI2C_CONF2_DCDC_SEL);
