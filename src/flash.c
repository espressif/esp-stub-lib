...
int stub_lib_flash_update_config(uint32_t flash_id, uint32_t flash_size, uint32_t block_size, uint32_t sector_size, uint32_t page_size, uint32_t status_mask)
{
    return stub_target_flash_update_config(flash_id, flash_size, block_size, sector_size, page_size, status_mask);
}
...
int stub_lib_flash_init(void **state)
{
    ...
    int result = stub_target_flash_update_config(flash_id, flash_size, STUB_FLASH_BLOCK_SIZE, STUB_FLASH_SECTOR_SIZE, STUB_FLASH_PAGE_SIZE, STUB_FLASH_STATUS_MASK);
    return result;
}
