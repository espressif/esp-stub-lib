...
int stub_lib_flash_read_buff(uint32_t addr, void *buffer, uint32_t size)
{
    ...
    if (large_flash_mode) {
        int res = stub_target_flash_4byte_read(FLASH_SPI_NUM, addr, buffer, size);
        STUB_LOG_TRACEF("(0x%x, 0x%x, %u) results: %d\n", addr, (uint32_t)buffer, size, res);
        return res;
    }
...
int stub_lib_flash_write_buff(uint32_t addr, const void *buffer, uint32_t size, bool encrypt)
{
    ...
    if (large_flash_mode) {
        int res = stub_target_flash_4byte_write(FLASH_SPI_NUM, addr, buffer, size, encrypt);
        STUB_LOG_TRACEF("(0x%x, 0x%x, %u) results: %d\n", addr, (uint32_t)buffer, size, res);
        return res;
    }
