...
/** Required alignment (bytes) for the stub_lib_slave_spi_rearm() buffer. */
 ...
 *
 * @param buf      Writable receive buffer, aligned to SLAVE_SPI_DMA_ALIGN
 *                 (cache-line-aligned on targets with cached DMA, e.g. 64 bytes
 *                 on ESP32-P4; 4-byte-aligned otherwise).
 ...
 *
 * @param data Pointer to frame bytes, 4-byte (word) aligned. TX is only read
 *             by DMA (writeback, never invalidate), so unlike the receive path
 *             it does not require cache-line alignment.
