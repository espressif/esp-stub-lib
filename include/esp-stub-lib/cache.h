...
/*
 * Runtime assumptions for cache/MMU APIs in this library:
 *
 *   - Stub code runs on core 0; no other code runs on the chip concurrently
 *     (no app, no ISRs, no other cores executing app code).
 *   - Caller is single-threaded and non-reentrant.
 *   - Caller code and data live in RAM, not in cached flash.
 *   - Each *_init() is paired with exactly one *_deinit().
 *   - No external agent races accesses through cache between init and deinit.
 *
 * Behavior is undefined if any of these don't hold.
 */
