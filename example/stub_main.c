...
static void __attribute__((unused)) test_slave_spi(void)
{
    uint8_t rx_buf[512] __attribute__((aligned(4)));
    size_t rx_len;

    (void)stub_lib_slave_spi_is_active();
    stub_lib_slave_spi_init();
    (void)stub_lib_slave_spi_take_rx_frame(&rx_len);
    (void)stub_lib_slave_spi_rearm(rx_buf, sizeof(rx_buf));
    (void)stub_lib_slave_spi_tx_frame(rx_buf, sizeof(rx_buf));
}
...
static int __attribute__((unused)) handle_test_flash(void)
{
    ...
    (void)stub_lib_flash_init_ex(flash_state, STUB_LIB_FLASH_ATTACH_IF_NEEDED, true);
